...
cl_context createContext() {
	...
	try {
		...
		if (!platformArray)
			throw runtime_error("Unable to create platform array");
		...
		if (clerr!=CL_SUCCESS || numPlatforms == 0)
			throw runtime_error("Unable to obtain platform information");
		...
		for (cl_uint i=0; i<numPlatforms; ++i) {
			...
			// Pick the first platform to be used
			platformIndex = 0;
		}
		...
		// Create OpenCL context based on the first available platform
		...
		if (clerr!=CL_SUCCESS || !context)
			throw runtime_error("Unable to create a valid GPU context");
